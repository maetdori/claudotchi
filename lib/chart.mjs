...
// via /claudotchi:config lifespan (or CLAUDOTCHI_LIFESPAN, 1–100). Stage boundaries
// are expressed as fractions of a lifespan, so the arc keeps the same shape at any
// lifespan value.
export const LIFESPAN_PCT = valueOf('lifespan');
...
export const NODES = {
  ...
  narutochi:       { name: '나루토치',   emoji: '🍥', stage: 'adult', grade: '★', hidden: true, profile: '히든 — 방치도치(낙제생)를 성실(⚡) 극대 + 깊은 교감(❤️)으로 되돌리면 각성하는 언더독' },
};
...
// Third hidden form: the underdog ninja (🍥 나루토치). Redemption route — a
// 방치도치(dead-last troublemaker, who reached that node *because* diligence was
// low early) is turned around: relentless attentive care pulls the diligence
// average back up AND a deep bond forms. So it flips into the hero instead of the
// zombie/oyaji dead-ends. diligence is read normalised (per prompt, like the tree),
// bond raw (like legend/nyang). Deterministic. Thresholds are tunable.
export function isNarutochiEligible(state) {
  const diligAvg = state.diligence / Math.max(1, state.promptCount);
  return diligAvg >= 0.6 && (state.bond || 0) >= 10;
}
...
function chooseChild(n, state) {
  ...
  else if ((childId === 'zombie' || childId === 'oyaji') && isNarutochiEligible(state)) childId = 'narutochi';
  return childId;
}
