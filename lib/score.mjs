...
//
// Default scoring is heuristic: free, instant, no tokens. If LLM scoring is on
// (/claudotchi:config llm on, or CLAUDOTCHI_LLM=1) and an ANTHROPIC_API_KEY is
// present, prompt quality is instead scored by a small model, falling back to the
// heuristic on any error/timeout.
...
export async function scorePromptLLM(prompt) {
  if (!valueOf('llm') || !process.env.ANTHROPIC_API_KEY) return null;
  try {
