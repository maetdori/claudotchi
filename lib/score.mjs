...
//
// Default scoring is heuristic: free, instant, no tokens. If TAMAGOTCHI_LLM=1
...
export async function scorePromptLLM(prompt) {
  if (process.env.TAMAGOTCHI_LLM !== '1' || !process.env.ANTHROPIC_API_KEY) return null;
  try {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: process.env.TAMAGOTCHI_LLM_MODEL || 'claude-haiku-4-5-20251001',
        max_tokens: 16,
        messages: [{
          role: 'user',
