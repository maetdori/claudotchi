...
//   node cli.mjs collection         -> list owned forms + a shareable web link
...
function family() {
  ...
  console.log('\n🏅 지금까지 모은 폼을 웹에서 보려면  →  /claudotchi:collection');
}

// The hosted collection book. `#coll=<ids>` auto-fills it with the owned set.
const WEB_URL = 'https://maetdori.github.io/claudotchi/';
// Every form the collection book tracks (12 normal + 3 hidden), in gallery order.
const COLLECTION_IDS = [
  'master', 'nerd', 'pro', 'model_citizen', 'basement_genius', 'glutton',
  'mypace', 'lazy', 'clown', 'berserk', 'zombie', 'oyaji',
  'legend', 'nyang', 'narutochi',
];

// The set of adult forms this user has ever reached — across living pets and
// every tombstone. Backs /claudotchi:collection and the shareable web link.
function collection() {
  const seen = new Set();
  for (const p of listAllPets()) if (p.currentForm) seen.add(p.currentForm);
  for (const t of allTombstones()) if (t.form) seen.add(t.form);
  const owned = COLLECTION_IDS.filter((id) => seen.has(id));
  const total = COLLECTION_IDS.length;

  console.log('🏅 내 클로도치 컬렉션북\n');
  if (!owned.length) {
    console.log('아직 어른 폼에 도달한 펫이 없어요. 세션을 좀 더 키운 뒤 다시 시도하세요.');
    console.log(`\n빈 컬렉션북 둘러보기 👉 ${WEB_URL}`);
    return;
  }
  owned.forEach((id) => {
    const n = node(id);
    console.log(`  ✓ ${n.emoji} ${n.name} (${gradeOf(id)}급)`);
  });
  console.log(`\n수집 현황: ${owned.length} / ${total}종`);
  console.log('\n웹 컬렉션북에서 내 수집 현황 보기 👇 (열면 자동으로 내 폼으로 채워져요)');
  console.log(`${WEB_URL}#coll=${owned.join(',')}`);
  console.log('\n(친구에게 이 링크를 공유하면 내 컬렉션을 그대로 볼 수 있어요.)');
}
