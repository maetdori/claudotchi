// cli.mjs — backs the /claudotchi:breed, :family and :sulk slash commands.
...
function sulk(arg) {
  ...
  let on, src;
  if (typeof cfg.sulk === 'boolean') { on = cfg.sulk; src = '슬래시 명령 설정'; }
  else if (process.env.CLAUDOTCHI_SULK !== undefined) { on = process.env.CLAUDOTCHI_SULK !== '0'; src = 'env CLAUDOTCHI_SULK'; }
  else { on = true; src = '기본값'; }
  console.log(`😤 삐짐 기능: ${on ? '켜짐' : '꺼짐'} (${src})`);
