...
// Minigames (😤 sulk, ⏱️ reaction, 🧠 quiz) are OFF by default — enable them with
// CLAUDOCHI_MINIGAMES=1 (also on/true/yes). Core growth (stat scoring from prompts
// and tools) is unaffected; only the surprise events + the sulk gate are toggled.
export const MINIGAMES_ON = ['1', 'on', 'true', 'yes'].includes(
  (process.env.CLAUDOCHI_MINIGAMES || '').toLowerCase(),
);
