...
function svg() {
  ...
  s += `<text x="${WIDTH / 2}" y="72" font-size="13" fill="${SUB}" text-anchor="middle">단계마다 다른 케어 스탯이 다음 모습을 결정해요 — 성장기 🧠지능 · 청년기 ⚡성실 · 성체 🧼청결 · 높을수록(高) 더 좋은 형태로</text>`;

  // left rail: stage names only (the driving stat now lives in the legend above
  // and as 高/중/低 tags on each branch — no more confusing duplicate label)
  const railX = 18;
  ...
  columns.forEach((col, c) => {
    ...
    col.teens.forEach((teen, t) => {
      ...
      // 청년기 → 2 성체 (청결 高 row / 청결 低 row), 🧼 tags on each drop
      s += connect(tcx, Y_TEEN + 42, tcx, Y_AD1 - 42, color);
      s += tag(tcx, (Y_TEEN + 42 + Y_AD1 - 42) / 2, '🧼 高', color, 10.5);
      s += connect(tcx, Y_AD1 + 42, tcx, Y_AD2 - 42, color);
      s += tag(tcx, (Y_AD1 + 42 + Y_AD2 - 42) / 2, '🧼 低', color, 10.5);
