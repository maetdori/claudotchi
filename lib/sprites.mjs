...
export function render(state, pct, nowMs = Date.now()) {
  ...
  // Two groups: identity (name + house) and status (bar + stats), one blank line
  // between them — roughly half the spacing of a gap-between-every-row. SPACER
  // (braille blank) renders empty yet isn't whitespace, so neither the per-line
  // left-trim nor the wrapper's trailing-newline strip eats it.
