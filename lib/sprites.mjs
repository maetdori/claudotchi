...
export function render(state, pct, nowMs = Date.now()) {
  ...
  // The sprite rows and the name stay tight together (they're the character).
  // The info rows below get a blank line between each so they aren't cramped.
  // SPACER (braille blank) renders empty yet isn't whitespace, so neither the
  // per-line left-trim nor the wrapper's trailing-newline strip eats it.
  const info = [
    houseLine(state),
    `${lifeBar(pct)}  ${dim(STAGE_LABEL[stage] || '')}`,
    statSummary(state),
  ];
  const ev = eventLine(state); if (ev) info.push(ev);
  const spacedInfo = info.flatMap((line) => [SPACER, line]); // blank line before each
