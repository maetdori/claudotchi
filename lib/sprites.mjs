...
export function render(state, pct, nowMs = Date.now()) {
  ...
  // Two groups: identity (name + house) and status (bar + stats), separated by a
  // faint dotted line instead of a full blank line — "half" spacing. SPACER
  // (braille blank) renders empty yet isn't whitespace, so neither the per-line
  // left-trim nor the wrapper's trailing-newline strip eats the top/bottom pad.
  const status = [`${lifeBar(pct)}  ${dim(STAGE_LABEL[stage] || '')}`, statSummary(state)];
  const ev = eventLine(state); if (ev) status.push(ev);
