...
import { MINIGAMES_ON } from './minigame.mjs';
...
function eventLine(state) {
  if (!MINIGAMES_ON) return null; // minigames disabled → no sulk/quiz/QTE banners
  ...
  // Show the actual question right here, so the user sees the quiz directly
  // instead of depending on the assistant to relay it. The next message answers.
  if (state.quizPending) return color(`🧠 깜짝 퀴즈: "${state.quizPending.q}" — 다음 메시지로 답해보세요!`, 'cyan');
  return null;
}
