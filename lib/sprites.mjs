...
export function render(state, pct, nowMs = Date.now()) {
  ...
  // Blank lines top and bottom give the pet breathing room from the surrounding
  // status line. SPACER (braille blank) renders empty yet isn't whitespace, so
  // neither the per-line left-trim nor the wrapper's trailing-newline strip eats it.
  const lines = [
