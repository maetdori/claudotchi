...
function identityLine(state, bodyRgb) {
  ...
  return `${badge}${name}${mood}   ${dim(`🏠 ${traits.family} · ${gen}세대${grade}`)}`;
}
...
export function render(state, pct) {
  ...
  // Blank lines top and bottom give the pet some breathing room from the
  // surrounding status line instead of cramming everything together. The spacer
  // is the GUARD escape, not an empty string, so the bottom one survives the
  // wrapper's `$(...)` capture (which strips trailing newlines but not the escape).
  const lines = [GUARD, ...art, identityLine(state, body), `${lifeBar(pct)}  ${dim(STAGE_LABEL[stage] || '')}`, statSummary(state)];
  const ev = eventLine(state); if (ev) lines.push(ev);
  lines.push(GUARD);
  return lines.join('\n');
}
