...
// a pixel gravestone — same grid the README hero banner uses, painted in the same
// truecolor block grammar as the living sprites (stone body + dark `k` engraving),
// so death looks hand-drawn like the rest instead of dropping to an ASCII slab.
export const STONE = [150, 138, 120];
export const GRAVE_GRID = ['·OOO·', 'OOOOO', 'OkkkO', 'OkOkO', 'OOOOO', 'OOOOO'];
...
function renderDead(state) {
  ...
  const grave = GRAVE_GRID.map((r, i) => paintRow(r, STONE, false, i, null));
  return [...Array(PAD_TOP).fill(SPACER), ...grave, color(head, 'red'), dim(hint)].join('\n');
}
