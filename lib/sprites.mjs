...
const EGG_RGB = [238, 228, 205];     // cream shell — the egg reads as an egg, not the mascot
const SPECKLE_RGB = [150, 110, 80];  // brown speckles on the shell
const RAINBOW = [
...
function paintRow(row, bodyRgb, eyes) {
  const B = fg(bodyRgb);
  const E = fg(EYE_RGB);
  const S = fg(SPECKLE_RGB);
  let out = '';
  for (const ch of row) {
    if (ch === 'O') out += `${B}██${RESET}`;
    else if (ch === 's') out += `${S}██${RESET}`;
...
export function render(state, pct) {
  ...
  const bodyRgb = stage === 'egg' ? EGG_RGB : (style.rgb || BASE_RGB);
