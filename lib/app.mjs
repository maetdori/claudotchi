...
function terminalMock() {
  ...
  <p class="term-cap">클로도치무료분양연락주세요01034788515</p>`;
}

// ── 메인: 생애주기 스트립 ─────────────────────────────────────────────────────────
const LIFE = [
  ['🥚', '알 · 0%'], ['🐣', '유아기'], ['🐤', '성장기'],
  ['🧒', '청년기'], ['🧑', '성체'], ['🪦', `죽음 · ${LIFESPAN_PCT}%`],
];
function lifeStrip() {
  return `<div class="life-strip">${LIFE.map(([e, l], i) =>
    `${i ? '<span class="arr">→</span>' : ''}<span class="lnode"><span class="bub">${e}</span><span class="lb">${l}</span></span>`
  ).join('')}</div>`;
}
...
const COLL = `
  ...
  <p class="coll-hint">데모: 카드를 눌러 획득/해제해 보세요. 실제 플러그인에선 <b>graveyard.json</b>에서 자동으로 채워져요.</p>
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ── app shell ── */
  .app{max-width:1200px;margin:0 auto;display:grid;grid-template-columns:1fr}
  @media(min-width:900px){.app{grid-template-columns:242px minmax(0,1fr);align-items:start}}
  .sidebar{position:sticky;top:0;z-index:40;background:rgba(20,17,14,.9);backdrop-filter:saturate(180%) blur(10px);border-bottom:1px solid var(--line)}
  @media(min-width:900px){.sidebar{height:100vh;border-bottom:0;border-right:1px solid var(--line);display:flex;flex-direction:column}}
  ...
  .content{padding:8px 34px 84px;min-width:0}
  ...
  /* ── buttons ── */
  .btn{display:inline-flex;align-items:center;gap:8px;padding:11px 19px;border-radius:12px;font:700 15px/1 inherit;
  ...
  .term-cap{text-align:center;color:var(--muted);opacity:.5;font-size:12px;margin-top:12px;max-width:760px}
  ...
  .life-strip .bub{width:50px;height:50px;border-radius:50%;background:var(--panel2);border:1px solid var(--line);display:grid;place-items:center;font-size:23px}
  ...
  /* growth-chart lightbox (fullscreen zoom/pan) */
  ...
  .lb-btn{width:42px;height:42px;border-radius:11px;border:1px solid var(--line);background:var(--panel);color:var(--ink);font:600 19px/1 inherit;cursor:pointer;display:grid;place-items:center;transition:background .12s}
  ...
  .tree-cta{display:flex;align-items:center;gap:8px;width:100%;max-width:760px;margin:0 0 16px;padding:14px 18px;
    ...
    font:600 14.5px/1.3 inherit;cursor:pointer;text-align:left;transition:border-color .12s,background .12s}
