...
function spriteFavicon(id) {
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < grid[y].length; x++) {
      ...
      if (col) px += `<rect x="${x + pad}" y="${y + pad}" width="1" height="1" fill="${rgb(col)}"/>`;
    }
  }
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W2} ${H2}" shape-rendering="crispEdges"><rect width="${W2}" height="${H2}" rx="1.5" fill="rgb(25,26,32)"/>${px}</svg>`;
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ── buttons ── */
  .btn{display:inline-flex;align-items:center;gap:8px;padding:11px 19px;border-radius:12px;font:800 15px/1 inherit;
  ...
  .term-cap{text-align:center;color:var(--clay);opacity:.92;font-size:13.5px;font-weight:700;letter-spacing:.01em;margin-top:12px;max-width:760px}
